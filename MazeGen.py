...
def getIndex(dim):
    index = int(dim[0] * n_cube + dim[1] * n_square + dim[2] * n + dim[3])
    return index
...
def traverseBFS():
    ...
    while len(visitedCells) > 0 and visitedCount != n_power_4:
        _mazeNeighbourCells = list(getNeighbours(
            currentVisitingMazeIndex))  # getNeighbours are the Neighbours of the current Visiting Maze Index
        ...
        if len(_mazeNeighbourCells) > 0:
            ...
            removeWalls(currentVisitingMazeIndex,
                        _neighbour)  # remove the walls between currentCell and next choosen neighbour
            visitedCount = visitedCount + 1
            visitedCells.append(_neighbour)
        else:
            visitedCells.pop()
            _neighbour = visitedCells[
                len(visitedCells) - 1]  # pick the last visited cell to be traversed next
